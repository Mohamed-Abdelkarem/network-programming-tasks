...
def rec():
    ...
    while True:
        w=s.recv(2048)
        Label(wind,text=w.decode('utf-8'),bd=5,relief=FLAT,bg='#d1c62e',padx=5,pady=5,wraplength=100).grid(column=0,row=r)  
        r=r+1
...
def clicked():
    ...
    x=en.get()
    Label(wind,text=x,bd=5,relief=FLAT,bg='#d12e44',padx=5,pady=5,wraplength=100).grid(column=2,row=r)  
    entryText.set("")
    r=r+1
btn["command"]=clicked
wind.mainloop()
